user_score_count = 0;
bot_score_count = 0;

function choose_option(user_option) {
    let bot_option = choose_bot_option();
    let bot_seletion = results = ``;

    if (user_option === bot_option) {
        // Tie match
        results += `Match tied`;
    }

    /* ########## User winning options ######### */
    else if (user_option === 0 && bot_option === 2) {
        // User = Rock Bot = Scissor
        results += `You Won`;
        user_score_count++;
    }
    else if (user_option === 1 && bot_option === 0) {
        // User = Paper Bot = Rock
        results += `You Won`;
        user_score_count++;
    }
    else if (user_option === 2 && bot_option === 1) {
        // User = Scissor Bot = Paper
        results += `You Won`;
        user_score_count++;
    }

    /* ########## Bot winning options ######### */
    else if (bot_option === 0 && user_option === 2) {
        // Bot = Rock User = Scissor
        results += `You Loose`;
        bot_score_count++;
    }
    else if (bot_option === 1 && user_option === 0) {
        // Bot = Paper User = Rock
        results += `You Loose`;
        bot_score_count++;
    }
    else if (bot_option === 2 && user_option === 1) {
        // Bot = Scissor User = Paper
        results += `You Loose`;
        bot_score_count++;
    }

    bot_seletion = bot_option_icon(bot_option);

    document.getElementById("bot_selection").innerHTML = bot_seletion;
    document.getElementById("results").innerHTML = results;
    document.getElementById("bot_score").innerHTML = bot_score_count;
    document.getElementById("user_score").innerHTML = user_score_count;
}

function choose_bot_option() {
    return Math.floor(Math.random() * 3);
}

function bot_option_icon(bot_option) {
    let icon = ``;
    if (bot_option === 0) {
        icon += `
        <svg enable-background="new 0 0 512 512" height="512" viewBox="0 0 512 512" width="512" xmlns="http://www.w3.org/2000/svg">
            <g>
                <path
                    d="m233.997 43.594c4.64-6.96 2.691-16.37-4.332-20.914l-31.33-20.272c-6.888-4.457-16.079-2.553-20.63 4.273l-38.794 58.19 60.675 30.337z" />
                <path
                    d="m287.304 139.067 28.402-42.603c4.64-6.96 2.691-16.37-4.332-20.914l-31.33-20.272c-6.888-4.457-16.079-2.553-20.63 4.273l-32.785 49.178z" />
                <path
                    d="m294.113 214.549 27.471 12.209c6.677 2.967 14.52.692 18.573-5.387l50.807-76.211c4.64-6.96 2.691-16.37-4.332-20.914l-24.879-16.099c-6.888-4.457-16.079-2.553-20.63 4.273l-53.4 80.1c-5.156 7.737-2.106 18.253 6.39 22.029z" />
                <path
                    d="m461.89 172.943-24.879-16.098c-6.888-4.457-16.079-2.553-20.63 4.273l-45.557 68.336c-5.157 7.736-2.107 18.252 6.389 22.028l27.471 12.209c6.677 2.967 14.52.693 18.573-5.387l42.965-64.448c4.64-6.958 2.691-16.368-4.332-20.913z" />
                <path
                    d="m410.771 294.986c-.002 0-.001 0-.003 0-6.33 0-12.477-1.306-18.269-3.881l-140.176-62.299h-9.479l63.013 126.025c3.812 7.625.521 16.943-7.364 20.433-7.386 3.269-16.046-.171-19.658-7.395l-69.532-139.063h-64.172c-8.077 0-15.027-6.207-15.407-14.275-.406-8.614 6.458-15.725 14.983-15.725h94.744c5.015 0 9.699-2.507 12.481-6.68l9.177-13.766c5.005-7.507 2.297-17.702-5.773-21.737l-121.35-60.675c-6.766-3.383-14.993-1.198-19.189 5.096l-69.021 103.533c-3.51 5.264-3.339 12.165.427 17.249l110.557 149.251c2.67 3.604 3.587 8.214 2.499 12.566l-27.43 109.718c-2.367 9.467 4.794 18.638 14.552 18.638h226.859c8.284 0 15-6.716 15-15v-80.061c0-5.007 2.498-9.683 6.659-12.467 0 0 3.763-2.517 9.33-6.229 24.678-16.451 39.411-43.979 39.411-73.638v-43.904c-8.434 9.041-20.267 14.286-32.869 14.286z" />
            </g>
        </svg>
        `;
    }
    else if (bot_option === 1) {
        icon += `
        <svg height="456pt" viewBox="-65 0 455 456" width="456pt" xmlns="http://www.w3.org/2000/svg">
            <path d="m76.320312 456h98.015626c27.351562 0 52.660156-15.882812 66.046874-41.601562l84.785157-162.835938-4.785157-3.917969c-.089843-.078125-.179687-.15625-.265624-.234375-7.367188-6.929687-17.28125-10.488281-27.371094-9.816406-10.304688.785156-19.820313 5.820312-26.269532 13.898438l-24.457031 29.773437c-1.875 2.28125-4.984375 3.140625-7.765625 2.144531-2.785156-.996094-4.640625-3.628906-4.644531-6.585937l-.140625-204.832031c-.011719-13.960938-10.410156-25.316407-23.183594-25.316407-12.492187 0-22.699218 10.828125-23.171875 24.34375 0 20.6875-.011719 43.207031-.015625 64.640625-.007812 48.476563-.015625 90.316406 0 94.375.007813 1.851563-.726562 3.628906-2.035156 4.9375-1.3125 1.304688-3.089844 2.035156-4.941406 2.027344h-.023438c-1.847656.015625-3.621094-.707031-4.9375-2.003906-1.3125-1.296875-2.054687-3.066406-2.0625-4.910156-.015625-4.085938-.007812-45.902344 0-94.402344 0-20.921875.007813-42.890625.011719-63.171875-.011719-.164063-.019531-.316407-.019531-.480469 0-.398438.007812-.78125.019531-1.179688 0-17.027343 0-32.753906 0-45.628906.007813-13.957031-10.394531-25.222656-23.175781-25.222656h-.710938c-12.78125 0-23.386718 11.175781-23.386718 25.144531v204.972657c0 3.863281-3.136719 7-7 7-3.867188 0-7-3.136719-7-7v-178.472657c0-13.96875-10.183594-25.332031-22.964844-25.332031-12.855469 0-23.035156 11.363281-23.035156 25.332031v178.472657c0 3.863281-3.136719 7-7 7-3.867188 0-7-3.136719-7-7v-105.203126c0-13.972656-10.628907-24.914062-23.414063-24.914062h-.667969c-12.75 0-23.152344 11.074219-23.183594 25-.132812 57.488281-.050781 158.84375.035157 249.570312.039062 44.953126 34.007812 81.429688 75.714843 81.429688zm0 0" />
        </svg>
        `;
    }
    else if (bot_option === 2) {
        icon += `
        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 287.084 287.084" style="enable-background:new 0 0 287.084 287.084;" xml:space="preserve">
            <g>
                <path d="M215.494,239.917l-100.187,0c-5.126,0-9.282,4.156-9.282,9.282v28.602c0,5.126,4.156,9.282,9.282,9.282h100.187
                c5.126,0,9.282-4.156,9.282-9.282v-28.603C224.776,244.073,220.62,239.917,215.494,239.917z" />
                                <path d="M141.574,223.32h39.537c24.077,0,43.665-19.589,43.665-43.666c0-6.332,0-24.859,0-31.179
                c-3.597,2.441-8,3.903-12.717,3.903c-6.391,0-12.303-2.642-16.507-7.099c-4.205,4.457-10.118,7.099-16.508,7.099
                c-13.801,0-21.387-10.643-21.343-15.602c0.007-0.758-0.035-5.792-0.035-34.91c0-2.766,2.304-4.871,4.871-4.871
                c2.69,0,4.871,2.181,4.871,4.871v32.227c0,0.951,0.276,1.884,0.796,2.68c0.001,0.001,0.001,0.002,0.002,0.003
                c2.367,3.619,6.397,5.859,10.837,5.859s8.47-2.24,10.838-5.859c0.515-0.788,0.784-1.697,0.784-2.637v-22.273
                c0-2.582,2.151-4.871,4.871-4.871c2.69,0,4.871,2.181,4.871,4.871v22.181c0,0.966,0.28,1.914,0.808,2.722
                c0.001,0.002,0.003,0.005,0.005,0.007c2.367,3.619,6.397,5.859,10.837,5.859c6.155,0,11.507-4.41,12.717-10.404v-30.365
                c0-9.105-7.382-16.487-16.487-16.487c-5.926,0-11.107,3.137-14.013,7.831v-5.357c0-9.105-7.382-16.487-16.487-16.487
                c-5.927,0-11.108,3.138-14.014,7.832v-62.71C163.775,7.382,156.393,0,147.288,0C138.182,0,130.8,7.382,130.8,16.487v65.28
                c0,1.168-0.91,2.133-2.076,2.201c-1.166,0.068-2.183-0.784-2.319-1.943l-6.245-52.997c-1.065-9.042-9.261-15.513-18.304-14.444
                c-9.043,1.065-15.51,9.261-14.444,18.304l8.181,69.434c7.706-2.385,15.624-0.478,21.318,4.606
                c8.636,7.707,9.854,20.687,2.91,29.849c-0.363,0.479-0.734,0.954-1.138,1.407l-15.026,16.833c-1.53,1.711-4.272,1.98-6.103,0.347
                c-1.781-1.59-1.936-4.322-0.347-6.102l15.027-16.834c4.965-5.559,4.487-14.084-1.079-19.05c-5.549-4.954-14.072-4.5-19.05,1.078
                l-26.36,29.529c-4.815,5.393-4.539,13.575,0.628,18.629l0.125,0.128c33.275,35.366,34.706,37.042,42.708,44.871
                C119.364,216.813,128.465,223.32,141.574,223.32z" />
            </g>
        </svg>
        `;
    }

    return icon;
}